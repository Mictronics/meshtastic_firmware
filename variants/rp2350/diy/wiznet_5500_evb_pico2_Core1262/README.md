# WIZnet W5500-EVB-Pico2 + Waveshare Core1262 - Meshtastic Variant

Meshtastic support for the **WIZnet W5500-EVB-Pico2**, a Raspberry Pi Pico 2 development board with an **integrated W5500 Ethernet PHY** on SPI0, paired with an external **Waveshare Core1262** LoRa module on SPI1.

This variant is hardware-twin of [`pico2_w5500_e22`](../pico2_w5500_e22/) (same LoRa pinout, same W5500 pin mapping) but targets the WIZnet PCB rather than a standalone Pico 2 + external W5500 module. The two key differences are:

|              | `pico2_w5500_e22`                      | `wiznet_5500_evb_pico2_Core1262`         |
| ------------ | -------------------------------------- | ---------------------------------------- |
...
| LoRa      | Waveshare Core1262     | SX1262, 868 MHz (Europe band)                                |
...
### Waveshare Core1262 LoRa (SPI1, external)

| Core1262 signal | RP2350 GPIO | Notes                                      |
| --------------- | ----------- | ------------------------------------------ |
| SCK             | GP10        | SPI1 clock                                 |
| MOSI            | GP11        | SPI1 TX                                    |
| MISO            | GP12        | SPI1 RX                                    |
...
The Waveshare Core1262 does **not** connect DIO2 to the TXEN pin of its PA internally. They must be bridged with a short wire or solder bridge **on the module itself**:
...
With this bridge in place, `SX126X_DIO2_AS_RF_SWITCH` causes the SX1262 to drive DIO2 HIGH automatically during TX, without needing an RP2350 GPIO for TXEN.
...
```bash
pio run -e wiznet_5500_evb_pico2_Core1262
```
...
```text
.pio/build/wiznet_5500_evb_pico2_Core1262/firmware-wiznet_5500_evb_pico2_Core1262-*.uf2
```

Or directly with picotool:

```bash
pio run -e wiznet_5500_evb_pico2_Core1262 -t upload
```
...
| `SX126X_DIO3_TCXO_VOLTAGE 1.8` | Core1262 TCXO controlled by DIO3                            |
...
Mapped to `meshtastic_HardwareModel_RPIPICO2`.
